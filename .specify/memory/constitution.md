...
Change Type: MAJOR (Initial Constitution - adapted for MarkDownLinkCheck)
...
- Domain focus: Markdown link validation and health-check reporting
...
# MarkDownLinkCheck Markdown 連結檢測系統憲章
...
- **錯誤處理**: 必須處理邊界情況並提供清晰的例外處理（包含連結失效、逾時、網路錯誤等場景）
...
- **關鍵路徑測試**: 所有關鍵業務邏輯（Markdown 解析、連結擷取、HTTP 連結驗證、報告產生）必須有單元測試覆蓋
- **整合測試**: 頁面渲染、表單處理、連結檢測服務必須有整合測試
- **測試命名**: 遵循現有檔案的命名風格和大小寫規範，不使用 "Arrange"、"Act"、"Assert" 註解
- **相依性模擬**: 有效使用 Mock 隔離 HTTP 呼叫和外部相依項目，確保測試可重複執行
...
- **錯誤訊息**: 提供清晰、可操作的錯誤訊息（包含連結失效原因、HTTP 狀態碼說明）
- **驗證回饋**: 即時驗證回饋，明確指出問題和修正方法（使用 jQuery Validation）
- **檢測結果呈現**: 連結檢測報告必須清晰呈現狀態（有效/無效/重新導向/逾時），支援排序與篩選
...
- **連結檢測效能**: 批次連結檢測必須使用並發 HTTP 請求（Parallel/async），避免序列阻塞
- **靜態資源最佳化**: CSS/JS 壓縮、圖片最佳化、適當使用 CDN 載入函式庫
- **記憶體管理**: 適當使用資源清理和 `IDisposable`，`HttpClient` 必須透過 `IHttpClientFactory` 管理
- **非同步程式設計**: 所有 HTTP 連結驗證和 I/O 密集操作必須使用 async/await 模式
- **快取策略**: 對近期檢測結果實施適當快取機制，避免重複檢測相同連結
- **效能監控**: 使用 Application Insights 或類似工具追蹤效能指標
- **大型文件處理**: 含大量連結的 Markdown 文件必須在合理時間內完成檢測，避免阻塞 UI
...
- **輸入驗證**: 所有使用者輸入必須驗證（使用 Data Annotations 或 FluentValidation），防止惡意 Markdown 或惡意 URL
- **SSRF 防護**: 連結檢測時必須限制可存取的內部網段，防止伺服器端請求偽造（SSRF）攻擊
- **XSS 防護**: 使用 Razor 引擎內建的 HTML 編碼，顯示連結 URL 和錯誤訊息時避免直接輸出未編碼內容
- **CSRF 防護**: 所有表單（如上傳 Markdown 內容）必須包含 Anti-Forgery Token
- **敏感資料保護**: 金鑰、連線字串等必須使用 Secret Manager 或環境變數
- **HTTPS Only**: 生產環境強制使用 HTTPS，啟用 HSTS
- **Content Security Policy**: 實施適當的 CSP 標頭防止注入攻擊

**理由**: 安全漏洞造成的損害遠超過預防成本，內建安全性比事後補強更有效且成本更低。連結檢測功能具有 SSRF 特殊風險，必須特別防範。
...
- **HTTP 連結驗證**: `IHttpClientFactory` + `HttpClient`（具名或具型別客戶端）
...
- **相依性注入**: 使用內建 DI 容器管理服務生命週期，連結檢測服務以 `ILinkCheckService` 介面定義
...
3. **憲章檢查**: 驗證設計符合本憲章所有原則，特別確認 SSRF 安全防護設計
4. **測試先行**: 撰寫失敗測試（含 HTTP Mock 測試）
5. **實作**: 按使用者故事優先級實作功能
6. **測試通過**: 確保所有測試通過
...
- ✅ 所有自動化測試通過（單元 + 整合）
- ✅ 程式碼覆蓋率 > 80%（關鍵業務邏輯：Markdown 解析、連結驗證）
- ✅ 無編譯警告或 linter 錯誤
- ✅ XML 文件註解完整（公開 API）
- ✅ 安全性掃描無高危漏洞（特別關注 SSRF）
- ✅ 至少一位團隊成員審查通過
...
**版本**: 1.0.0 | **批准日期**: 2026-02-12 | **最後修訂**: 2026-02-23
